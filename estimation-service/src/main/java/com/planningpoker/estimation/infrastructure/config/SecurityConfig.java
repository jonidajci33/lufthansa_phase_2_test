...
/**
 * Spring Security configuration for the Estimation Service.
 * Configures JWT-based authentication with Keycloak realm role mapping.
 * <p>
 * WebSocket endpoints are permitted at HTTP level; authentication is handled
 * by {@link WebSocketAuthInterceptor} on STOMP CONNECT frames.
 */
@Configuration
@EnableWebSecurity
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // WebSocket — auth handled by STOMP interceptor
                        .requestMatchers("/ws/**").permitAll()
                        // Swagger / OpenAPI
                        .requestMatchers("/api-docs/**", "/swagger-ui/**", "/swagger-ui.html").permitAll()
                        // Admin-only: list all stories across all rooms
                        .requestMatchers(HttpMethod.GET, "/api/v1/stories").hasRole("ADMIN")
                        // All other estimation API endpoints require authentication
